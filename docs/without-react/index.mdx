...
The following tutorial shows how to build a simple game and displaying a player tag from the [simple-game]() and [access-avatar-and-profile]() tutorials using only three.js.
...
```bash
npm init -y
npm install three @pmndrs/viverse @pmndrs/uikit @viverse/sdk @pixiv/three-vrm
npm install -D vite
```
...
Add a realistic sky background using Three.js Sky:
...
## Step 5: Adding Physics and Character
...
const character = new SimpleCharacter(camera, world, canvas, { 
  model: { url: profile.activeAvatar?.vrmUrl } 
})
```

Create a `.env.prod` file for your Viverse app ID:
...
> [!WARNING]
> Only apply the `.env.prod` environment when building the application and not during local development
