...
Now let's build our game scene step by step. We'll start simple and add complexity gradually so you can see the progress at each stage.

## Step 1: Adding the Sky

Let's start with just the sky and basic lighting. Add the `Sky` import from `@react-three/drei`:
